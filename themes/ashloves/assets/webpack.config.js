...
// JS Directory path.
const JS_DIR = path.resolve(__dirname, "src") + "/js/pages/";
const JS_COMP_DIR = path.resolve(__dirname, "src") + "/js/components/";
...
const plugins = argv => {
  if("production" === argv.mode) {
    ...
    const webpackSettings = require("./webpackSettings.js");
    return [
      new CleanWebpackPlugin({
        cleanStaleWebpackAssets: "production" === argv.mode // Automatically remove all unused webpack assets on rebuild, when set to true in production. ( https://www.npmjs.com/package/clean-webpack-plugin#options-and-defaults-optional )
      }),
      ...
      new BrowserSyncPlugin( {
        proxy: webpackSettings.localhost,
        files: [
          '**/*.php'
        ],
        reloadDelay: 0
